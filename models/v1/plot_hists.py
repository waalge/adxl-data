...
if __name__ == "__main__":
    ...
    pwm_plot_values = np.array(
        np.linspace(int(np.min(pwm_values)), int(np.max(pwm_values)), num_pwm_values),
        dtype="int",
    )
    ...
    fig, ax = plt.subplots(
        side_length, side_length, figsize=(4 * side_length, 4 * side_length)
    )
    ...
    for plotx, pwm_value in enumerate(pwm_plot_values):
        ...
        for ix in indices:
            ...
            ax[axx][axy].bar(
                bars, pp_accel_readings[ix, :], width=widths, color="b", alpha=0.1
            )
